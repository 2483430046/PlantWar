...
GAME_OVER = False
# 4. 图片加载报错处理
LOG = '文件:{}中的方法:{}出差'.format(__file__,__name__)
...
class PeaBullet(pygame.sprite.Sprite):
    def __init__(self, peashooter):
        self.live = True
        self.image = pygame.image.load("images/peabullet.png")
        self.damage = 200
        self.speed = 10
        self.rect = self.image.get_rect()
    ...
    def nextLevel(self):
        MainGame.score += 20
        MainGame.remain_score -= 20
        for i in range(1, 100):
            if MainGame.score == 100*i and MainGame.remain_score==0:
                MainGame.remain_score = 100 * i
                MainGame.shaoguan += 1
...
class MainGame():
    ...
    def draw_text(self, content, size, color):
        # 初始化字体模块
        pygame.font.init()
        font = pygame.font.Font("/usr/share/fonts/opentype/noto/NotoSansCJK-Bold.ttc", size)
        # font = pygame.font.SysFont("NotoSansCJK-Bold", size)
    ...
    def start_game(self):
        ...
        while not GAME_OVER:
            ...
            MainGame.window.blit(self.draw_text(
                "当前关数{}, 得分{},距离下关还差{}分".format(MainGame.shaoguan, MainGame.score, MainGame.remain_score), 26,
                (255, 0, 0)), (5, 40))
            self.load_help_text()  #6 调用加载植物的方法
            pygame.mixer.init()
